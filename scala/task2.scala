...
object task2 {
  ...
  def filterBaskets(baskets: Array[Array[String]], freq_singles: Set[String]) : Array[Array[String]] = {
    var newBaskets = new ArrayBuffer[Array[String]]()

    for (basket <- baskets) {
      newBaskets += basket.toSet.intersect(freq_singles).toArray
    }
    return newBaskets.toArray
  }

  def multihash(baskets: Array[Array[String]], s: Int, totalItems: Long): Iterator[Set[String]] = {
    val support = math.ceil(s * baskets.length / totalItems.toFloat).toInt + 1
    ...
    val baskets2 = filterBaskets(baskets, l)

    breakable {
      while (true) {
        var ckp1 = getCFromL(baskets2, temp, k)
        println("Processed C for k="+k)
        var lkp1 = getLFromC(ckp1, support)
        println("Processed L for k="+k)
        ckp1 = null
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rdd = textRDD.filter(x => x != first)
      ...
      .filter(x => (x.length > filter_threshold))
